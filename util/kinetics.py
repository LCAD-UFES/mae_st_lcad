...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.pretrain_jitter_scales_relative = pretrain_jitter_scales_relative
        self.pretrain_jitter_aspect_relative = pretrain_jitter_aspect_relative
        ...
        # TODO: use PTV augmentation
        # self.pretrain_aug = self.get_pretrain_augmentation()
    ...
    def dummy__getitem__(self, index):
        return torch.zeros((3, 16, 224, 224)), torch.ones((1))
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["pretrain", "finetune", "val"]:
            ...
            min_scale, max_scale, crop_size = (
                ...
                if self._test_num_spatial_crops > 1
                else [self._train_jitter_scales] * 2 + [self._test_crop_size]
            )
        ...
        # print(f"Kinetics loader mode {self.mode} rand aug {self.rand_aug} min_scale {min_scale} max_scale {max_scale} crop_size {crop_size}")
    ...
    def _aug_frame(
        ...
        # Perform data augmentation.
        scl, asp = (
            self.pretrain_jitter_scales_relative,
            self.pretrain_jitter_aspect_relative,
        )
        relative_scales = (
            None if (self.mode not in ["pretrain"] or len(scl) == 0) else scl
        )
        relative_aspect = (
            None if (self.mode not in ["pretrain"] or len(asp) == 0) else asp
        )
    ...
    def get_pretrain_augmentation(
        ...
        transform = Compose(
            [
                ...
                RandomShortSideScale(
                    min_size=self._train_jitter_scales[0],
                    max_size=self._train_jitter_scales[1],
                ),
                RandomCropVideo(self._train_crop_size),
